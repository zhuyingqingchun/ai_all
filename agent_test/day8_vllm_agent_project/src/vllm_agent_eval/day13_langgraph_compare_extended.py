...
def run_langgraph(dataset_path: Path, output_dir: Path) -> dict:
...
def run_day13_compare(project_root: Path, dataset_path: Path, model: str, output_dir: Path) -> dict:
    output_dir.mkdir(parents=True, exist_ok=True)

    baseline = run_baseline(project_root=project_root, dataset_path=dataset_path, model=model, output_dir=output_dir)
    langgraph = run_langgraph(dataset_path=dataset_path, output_dir=output_dir)
    ...
    summary_path = output_dir / "day13_compare_summary.json"
