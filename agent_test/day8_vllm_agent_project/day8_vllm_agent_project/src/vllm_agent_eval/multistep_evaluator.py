...
CONNECTOR_RE = re.compile(r"(?:，|,|；|;)?(?:先|再|然后|接着|并且|并|最后)")


def normalize_clause(text: str) -> str:
    text = text.strip().strip("，,。；; ")
    for prefix in ["先", "再", "然后", "接着", "并且", "并", "最后"]:
        if text.startswith(prefix):
            text = text[len(prefix):].strip()
    return text


def heuristic_split_query(user_input: str) -> List[str]:
    text = user_input.strip()
    if not any(k in text for k in ["先", "再", "然后", "最后", "并且", "并"]):
        return []
    parts = [normalize_clause(p) for p in CONNECTOR_RE.split(text) if normalize_clause(p)]
    if len(parts) >= 2:
        return parts
    # 兼容“先A，再B”未被前面切好的情况
    parts = [normalize_clause(p) for p in re.split(r"(?:，|,|；|;)?(?:再|然后|接着|最后)", text) if normalize_clause(p)]
...
def heuristic_multistep_plan(user_input: str, state: ConversationState) -> Optional[Dict[str, Any]]:
    clauses = heuristic_split_query(user_input)
    if len(clauses) < 2:
        return None
    ...
    non_tool_clauses: List[str] = []

    for clause in clauses:
        single_plan = safe_parse_plan("", clause, temp_state)
        if single_plan["tool"] == "direct_answer":
            non_tool_clauses.append(clause)
            continue
        step = {
            "tool": single_plan["tool"],
            "args": single_plan.get("args", {}),
            "purpose": clause,
        }
        steps.append(step)
        temp_state.update(single_plan)
    ...
    return {
        "steps": steps,
        "final_instruction": "；".join(non_tool_clauses) if non_tool_clauses else user_input,
        "route_source": "heuristic_multistep",
    }
...
def build_multistep_plan(chat_client: VLLMChatClient, context: List[Dict[str, str]], user_input: str, state: ConversationState) -> Tuple[Dict[str, Any], str, float]:
    heuristic_plan = heuristic_multistep_plan(user_input, state)
    if heuristic_plan is not None:
        return heuristic_plan, json.dumps(heuristic_plan, ensure_ascii=False), 0.0
    try:
        ...
        pass
    single_plan = safe_parse_plan("", user_input, state)
    if single_plan["tool"] == "direct_answer":
        return {
            "steps": [],
            "final_instruction": user_input,
            "route_source": "fallback_single_direct_answer",
        }, json.dumps(single_plan, ensure_ascii=False), 0.0
    return {
        "steps": [{"tool": single_plan["tool"], "args": single_plan.get("args", {}), "purpose": user_input}],
        "final_instruction": user_input,
        "route_source": "fallback_single_step",
    }, json.dumps(single_plan, ensure_ascii=False), 0.0
    ...
    def run_dataset(self, dataset: Sequence[Dict[str, Any]]) -> List[MultiStepTurnTrace]:
        ...
        for session in dataset:
            ...
            for index, turn_spec in enumerate(session["turns"], start=1):
                trace = self.run_turn(session["session_name"], index, turn_spec, memory, state)
